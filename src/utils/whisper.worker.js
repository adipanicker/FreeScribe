...
async function transcribe(audio) {
  ...
  await pipeline(audio, {
    ...
    callback_function:
      generationTracker.callback_function.bind(generationTracker),
...
async function load_model_callback(data) {
  const { status } = data;
  if (status === "progress") {
    const { file, progresss, loaded, total } = data;
    sendDownloadingMessage(file, progresss, loaded, total);
  }
}

function sendLoadinMessage(status) {
  self.postMessage({
...
class generationTracker {
  ...
  chunkCallback(data) {
    ...
    this.processed_chunks = chunks.map((chunk, index) => {
      return this.processChunks(chunk, index);
    });
